...
class BinanceClientWrapper:
    ...
    def __init__(self):
        ...
        # Import global_config inside the method to avoid circular imports
        from src.config.global_config import global_config
        self.global_config = global_config
        
        self.is_futures = self.global_config.BINANCE_FUTURES
    ...
    def _initialize_client(self):
        ...
        try:
            if self.global_config.BINANCE_TESTNET:
                if self.is_futures:
                    # Use futures testnet
                    self.client = Client(
                        api_key=self.global_config.BINANCE_API_KEY,
                        api_secret=self.global_config.BINANCE_SECRET_KEY,
                        testnet=True
                    )
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            if self.is_futures:
                ...
                env = "FUTURES TESTNET" if self.global_config.BINANCE_TESTNET else "FUTURES MAINNET"
                url = self.client.FUTURES_URL if self.global_config.BINANCE_TESTNET else "https://fapi.binance.com"
            else:
                # Test spot connection
                self.client.ping()
                env = "SPOT TESTNET" if self.global_config.BINANCE_TESTNET else "SPOT MAINNET"
